import React, { useEffect, useRef, useState } from "react";
import { Helmet } from "react-helmet";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import {
  FiCpu,
  FiMessageSquare,
  FiGitBranch,
  FiSliders,
  FiUsers,
  FiActivity,
  FiZap,
  FiSettings
} from "react-icons/fi";

gsap.registerPlugin(ScrollTrigger);

// === Generative AI Services ===
const genAIServices = [
  {
    title: "Custom LLM Development",
    description:
      "Train or fine-tune large language models (LLMs) like GPT, LLaMA, or Falcon for your domain-specific needs.",
    icon: FiCpu,
    color: "from-purple-600 to-indigo-500",
  },
  {
    title: "Content & Code Generation",
    description:
      "Automate content creation (blogs, social, marketing) or generate code snippets with GenAI-powered assistants.",
    icon: FiZap,
    color: "from-blue-600 to-cyan-500",
  },
  {
    title: "Prompt Engineering",
    description:
      "Design and test optimized prompts for LLMs to ensure reliable and contextually accurate responses.",
    icon: FiSliders,
    color: "from-pink-600 to-rose-500",
  },
  {
    title: "AI Workflow Integration",
    description:
      "Embed GenAI into apps or internal tools via APIs or pipelines (e.g., LangChain, Semantic Kernel).",
    icon: FiGitBranch,
    color: "from-sky-600 to-blue-400",
  },
];

// === Chatbot Services ===
const chatbotServices = [
  {
    title: "Custom AI Chatbots",
    description:
      "Build intelligent chatbots for websites, apps, or WhatsApp using OpenAI, Rasa, or Dialogflow.",
    icon: FiMessageSquare,
    color: "from-green-600 to-emerald-500",
  },
  {
    title: "Multilingual Support Bots",
    description:
      "Deploy bots that support real-time translations and multiple languages using LLMs and translation APIs.",
    icon: FiUsers,
    color: "from-orange-500 to-yellow-400",
  },
  {
    title: "Conversational Analytics",
    description:
      "Track bot performance, user intent, drop-off rates, and improve with data-driven conversation insights.",
    icon: FiActivity,
    color: "from-yellow-600 to-amber-400",
  },
  {
    title: "CRM & Tool Integration",
    description:
      "Connect bots to CRMs, databases, or Slack/Teams using custom logic and Webhooks/API integrations.",
    icon: FiSettings,
    color: "from-red-600 to-pink-500",
  },
];

const GenAIChatbots = () => {
  const [selectedGenAI, setSelectedGenAI] = useState(null);
  const [selectedChatbot, setSelectedChatbot] = useState(null);
  const sectionRef = useRef(null);

  useEffect(() => {
    const section = sectionRef.current;
    gsap.fromTo(
      section.querySelectorAll(".gsap-card"),
      {
        y: 50,
        opacity: 0,
      },
      {
        y: 0,
        opacity: 1,
        duration: 0.8,
        stagger: 0.15,
        ease: "power3.out",
        scrollTrigger: {
          trigger: section,
          start: "top 80%",
        },
      }
    );
  }, []);

  const renderCards = (data, selected, setSelected) =>
    data.map((service) => (
      <div
        key={service.title}
        className={`gsap-card break-inside-avoid mb-6 p-5 rounded-xl transition-all duration-300 border border-gray-700/40 group cursor-pointer relative ${
          selected?.title === service.title
            ? "bg-gradient-to-br scale-[1.02] shadow-xl"
            : "hover:bg-gray-800/30 shadow-md bg-gray-900/40"
        } ${service.color}`}
        onClick={() =>
          setSelected((prev) => (prev?.title === service.title ? null : service))
        }
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg bg-gradient-to-br ${service.color} mr-3`}>
              <service.icon className="text-xl text-white" />
            </div>
            <h2
              className={`text-md font-semibold ${
                selected?.title === service.title
                  ? "text-white"
                  : "text-gray-300 group-hover:text-white"
              }`}
            >
              {service.title}
            </h2>
          </div>
          <span
            className={`text-xl ${
              selected?.title === service.title
                ? "text-white rotate-45"
                : "text-gray-400 group-hover:text-white"
            } transition-transform`}
          >
            +
          </span>
        </div>

        {selected?.title === service.title && (
          <div className="mt-3 text-sm text-gray-300 leading-relaxed animate-fadeIn">
            {service.description}
          </div>
        )}
      </div>
    ));

  return (
    <>
      <Helmet>
        <title>GenAI & Chatbot Services</title>
        <meta
          name="description"
          content="Powerful generative AI and chatbot development — from custom LLMs to multilingual virtual assistants and analytics."
        />
      </Helmet>

      <div
        ref={sectionRef}
        className="min-h-[80vh] w-full bg-black text-white px-4 md:px-6 lg:px-10"
      >
        <div className="absolute z-0 h-screen flex justify-center items-center left-[43%] animate-pulse">
          <div className="h-[200px] w-[200px] bg-purple-300 opacity-50 blur-[100px]"></div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 max-w-7xl mx-auto relative">
          {/* GenAI */}
          <div className="flex flex-col justify-center items-center">
            <h3 className="text-xl font-semibold mb-4 text-black unbounded bg-gradient-to-br from-gray-500 via-white to-gray-300 px-4 py-2 rounded-full">
              Generative AI Services
            </h3>
            {renderCards(genAIServices, selectedGenAI, setSelectedGenAI)}
          </div>

          {/* Center Title */}
          <div className="hidden lg:flex flex-col justify-center items-center text-center px-4">
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-gray-400 via-gray-300 to-white bg-clip-text text-transparent unbounded mb-4">
              GenAI & Chatbot Services
            </h1>
            <p className="text-lg text-gray-300 max-w-md">
              Empower your business with custom AI assistants, fine-tuned LLMs, and intelligent chat workflows.
            </p>
          </div>

          {/* Chatbots */}
          <div className="flex flex-col justify-center items-center">
            <h3 className="text-xl font-semibold mb-4 text-black unbounded bg-gradient-to-br from-gray-500 via-white to-gray-300 px-4 py-2 rounded-full">
              Chatbot Services
            </h3>
            {renderCards(chatbotServices, selectedChatbot, setSelectedChatbot)}
          </div>
        </div>
      </div>
    </>
  );
};

export default GenAIChatbots;
